...
it('T19: dropAllTables keeps key_value + print_record rows, drops everything else', () => {
...
describe('T20: resetDatabase round-trip preserves/clears protected tables', () => {
  it("'session' preserves key_value + print_record rows across drop+rebuild", () => {
    const db = new DatabaseSync(':memory:')
    db.exec('PRAGMA foreign_keys = ON')
    applyDdl(adapter(db), FULL_DDL)
  ...
  it("'none' drops key_value + print_record ROWS entirely (tables rebuilt empty)", () => {
    const db = new DatabaseSync(':memory:')
    db.exec('PRAGMA foreign_keys = ON')
    applyDdl(adapter(db), FULL_DDL)
    db.prepare(`INSERT INTO key_value (key, value) VALUES (?, ?)`).run('binding', 'lg-9')
...
describe('T21: applyDdl surfaces real errors, idempotent replay stays quiet', () => {
  it('throws on a malformed statement instead of swallowing it', () => {
    const db = new DatabaseSync(':memory:')
    expect(() => applyDdl(adapter(db), 'CREATE TABLE "x" (this is not valid sql')).toThrow()
  })

  it('throws on a duplicate column rather than building a broken table', () => {
    const db = new DatabaseSync(':memory:')
    expect(() =>
  ...
  it('does NOT throw when replaying idempotent CREATE IF NOT EXISTS', () => {
    const db = new DatabaseSync(':memory:')
    expect(() => applyDdl(adapter(db), FULL_DDL)).not.toThrow()
...
it('T23: applying A → B → A converges to a consistent DB with the final uuid', () => {
  const db = new DatabaseSync(':memory:')
  db.exec('PRAGMA foreign_keys = ON')
  const probe = '\nCREATE TABLE IF NOT EXISTS "schema_sync_probe" ("id" INTEGER PRIMARY KEY);'

  migrateSchema(adapter(db), { schemaUuid: 'A', fullDdl: FULL_DDL })
  migrateSchema(adapter(db), { schemaUuid: 'B', fullDdl: FULL_DDL + probe })
  migrateSchema(adapter(db), { schemaUuid: 'A', fullDdl: FULL_DDL })
...
describe('T24: FULL_DDL executes fully and builds a consistent schema', () => {
  ...
  it('creates every expected table with no FK violations', () => {
    const db = new DatabaseSync(':memory:')
    db.exec('PRAGMA foreign_keys = ON')
    applyDdl(adapter(db), FULL_DDL)
  ...
  it('is idempotent: re-applying does not add or drop tables', () => {
    const db = new DatabaseSync(':memory:')
    db.exec('PRAGMA foreign_keys = ON')
    applyDdl(adapter(db), FULL_DDL)
...
it('setSchemaUuid/getSchemaUuid round-trip', () => {
  const db = new DatabaseSync(':memory:')
  db.exec('PRAGMA foreign_keys = ON')
  applyDdl(adapter(db), FULL_DDL)
