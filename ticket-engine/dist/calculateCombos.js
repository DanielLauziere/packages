export function calculateCombos(ticketMenuItems, comboComboMenuItems, menuItems) {
    let totalCents = 0;
    const comb = [];
    for (const comboComboMenuItem of comboComboMenuItems) {
        const existsIndex = comb.findIndex((c) => c.uuid === comboComboMenuItem.comboUuid);
        if (existsIndex === -1) {
            comb.push({
    ...
    for (const combo of comb) {
        ...
        while (hasComboItems) {
            ...
            for (const comboItem of combo.items) {
                const ticketItem = ticketMenuItems.find((t) => t.menuItemUuid === comboItem);
                if (!ticketItem) {
                    hasComboItems = false;
                    break;
                }
            ...
            if (hasComboItems) {
                ...
                for (const deleteUuid of deleteItems) {
                    ...
                    if (index !== -1) {
                        ...
                        if (!ticketItem)
                            continue;
                        const menuItem = menuItems.find((m) => m.uuid === ticketItem.menuItemUuid);
                        if (menuItem) {
                            comboMenuItems.push({
                ...
                const comboCombo = comboComboMenuItems.find((c) => c.comboUuid === combo.uuid);
                if (comboCombo) {
                    combos.push({
...
//# sourceMappingURL=calculateCombos.js.map